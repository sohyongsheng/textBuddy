...
public class TaskManager {
	...
	public String toString() {
		if (tasks.size() == 0) {
			...
			for (Task aTask : tasks) {
				...
				displayAllTasks = displayAllTasks + "\n" + taskUserIndex + ". " + aTask.getDescription();
			}
	...
	public static void parseCommandLines(TaskManager textBuddy) {
		...
		for (textBuddy.prompt(); commandLineScanner.hasNextLine(); textBuddy.prompt()) {
		    String commandLine = commandLineScanner.nextLine().replaceAll("\n", "");
		    String[] commandLineChunks = commandLine.split(" ", NUMBER_OF_PARTS_MAKING_COMMAND_LINE); 
	...
	public static void mapCommandsToFunctions(TaskManager textBuddy,
		...
		case "delete":
			int taskUserIndexToRemove = Integer.parseUnsignedInt(commandParameter);
